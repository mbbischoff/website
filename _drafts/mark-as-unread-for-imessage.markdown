...
Even if this slight ambiguity is too much for Apple to handle, there's already another perfectly fine solution they could reach for that also comes from Mail, flagging. Allow users to flag a conversation (or an individual message) and then see all their flags in one place, when they're ready to respond.
